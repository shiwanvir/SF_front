...
@Component({
  selector: 'app-inventory-plantwise-current',
  templateUrl: './inventory-plantwise-current.component.html',
  styleUrls: ['./inventory-plantwise-current.component.css'],
  providers: [DatePipe]
})
export class InventoryPlantwiseCurrentComponent implements OnInit {
  ...
  this.http.get(this.apiUrl + 'dashboard/dashboard?type=inventory-plantwise-currentyear&customer='+this._route.snapshot.queryParamMap.get('id')).subscribe(response => {
      var data = response;
