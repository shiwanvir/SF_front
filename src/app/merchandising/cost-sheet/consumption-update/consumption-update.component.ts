...
@Component({
  ...
  styleUrls: ['./consumption-update.component.css']
})
...
export class ConsumptionUpdateComponent implements OnInit {
  ...
  costing$: Observable<Style[]>;//use to load costing list in ng-select
  ...
  ngOnInit() {
    ...
    this.titleService.setTitle("Costing YY Update")//set page title
  ...
  getCostings() {
    this.costing$ = this.costingInput$
    .pipe(
       debounceTime(200),
       distinctUntilChanged(),
       tap(() => this.costingLoading = true),
       switchMap(term => this.http.get<Style[]>(this.apiUrl + 'merchandising/costing?type=auto' , {params:{search:term}})
       .pipe(
        ...
        afterChange : (changes, source) => {
          if(source != null && source.length > 0){
            ...
            if(source[0][1] == 'net_consumption'){
              ...
              let _wastage = hotInstance2.getDataAtCell(_row, 8)
              ...
              if(this.countDecimals(_net_consumption) > decimalCount){
                ...
                hotInstance2.setDataAtCell(_row, 6, _net_consumption)
              }
              ...
              hotInstance2.setDataAtCell(_row, 9, gross_consumption)
              ...
              let _net_consumption = hotInstance2.getDataAtCell(_row, 6)
            ...
            if(this.countDecimals(_wastage) > 4){
              ...
              hotInstance2.setDataAtCell(_row, 8, _wastage)
            }
    ...
    if(unsaved_items.length > 0){
      ...
      .subscribe(
        (res) => {
          ...
          AppAlert.closeAlert()
          //AppAlert.showSuccess({text : res['data']['message']})
          AppAlert.showSuccess({text : 'Costing YY Updated Successfully'})
        },
        error => {
          this.processing = false
          setTimeout(() => {
  ...
  calculate_item_cost(_row){
    const hotInstance2 = this.hotRegisterer.getInstance(this.tblRM);
    // gross consumption = net consumption + (net consumption * wastage) / 100
    let net_consumption = hotInstance2.getDataAtCell(_row, 6)
    ...
    let wastage = hotInstance2.getDataAtCell(_row, 8)
  ...
  search(){
    ...
    reqData.costing_id = (reqData.costing_id == undefined || reqData.costing_id == null) ? 0 : reqData.costing_id.id
  ...
  loadItems(reqData){
    let reqStr = `customer_id=${reqData.customer_id}&style_id=${reqData.style_id}&division_id=${reqData.division_id}&color_type_id=${reqData.color_type_id}
      ...
      &merchant=${reqData.merchant}&costing_id=${reqData.costing_id}&bom_stage_id=${reqData.bom_stage_id}`;

      //console.log(reqData)
      this.http.get<any[]>(this.apiUrl + 'merchandising/costing?type=get_items_for_yy_update&' + reqStr)
      .pipe(map(res => res['data']))
      .subscribe(
        res => {
          this.dataRM = res
  ...
  notifyMerchant(){
    ...
    for(let x = 0 ;  x < this.dataRM.length ; x++){
      if(this.dataRM[x]['edited'] == true){
        hasUnsavedData = true
        break
      }
      if(!costingIdList.includes(this.dataRM[x]['costing_id'])){
        costingIdList.push(this.dataRM[x]['costing_id'])
      }
    }

    //chek for unsaved data, and then exit from the function
    if(hasUnsavedData == true){
      AppAlert.closeAlert()
      AppAlert.showError({ text : 'You have unsaved changes.' })
      this.processing = false
      return
    }

    this.http.post(this.apiUrl + 'merchandising/costing/notify-merchant', {costing_id_list : costingIdList})
    .subscribe(
      res => {
        this.processing = false
        AppAlert.closeAlert()
