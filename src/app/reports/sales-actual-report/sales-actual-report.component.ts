...
@Component({
  selector: 'app-sales-actual-report',
  templateUrl: './sales-actual-report.component.html'
})
export class SalesActualReportComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.titleService.setTitle("Sales Actual Report")
  ...
  reset_feilds() {
    ...
    var table = $('#sales_actual_tbl').DataTable();
    table
    .clear()
    .draw();
  }
  ...
  searchFrom() {
    $('#sales_actual_tbl').DataTable().destroy();
    ...
    $('#sales_actual_tbl tfoot th').each( function () {
      var title = $(this).text();
      $(this).html('<input type="text" class="form-control input-xxs" placeholder="'+title+'"/>' );
    });

    this.datatable = $('#sales_actual_tbl').DataTable({
      autoWidth: false,
      scrollY: "500px",
      scrollX: true,
      scrollCollapse: true,
      ...
      ajax: {
        ...
        dataType : 'JSON',
        "url": this.apiUrl + "reports/load-sales-actual",
        beforeSend : function() {
