...
@Component({
  selector: 'app-non-inventory-po-list',
  templateUrl: './non-inventory-po-list.component.html',
  styleUrls: []
})
export class NonInventoryPoListComponent implements OnInit {
  ...
  createTable() { //initialize datatable
    this.datatable = $('#non_inventory_po_list').DataTable({
      autoWidth: false,
      scrollY: "500px",
      scrollX: true,
      scrollCollapse: true,
      ...
      ajax: {
        ...
        dataType : 'JSON',
        "url": this.apiUrl + "merchandising/po_manual_header_non?type=datatable",
        data : {
    ...
    $('#non_inventory_po_list').on('click','i',e => {
  ...
  edit(data,confm){
    this.manualPoService.changeData2(data,confm)
  }
  ...
  delete(data) {
    ...
    (result) => {
      if (result.value) {
        this.http.post(this.apiUrl + 'merchandising/po_manual_header/cancel_po_2' , { 'po_id' : data } )
