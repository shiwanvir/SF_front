...
@Component({
  selector: 'app-current-ld-plantwise',
  templateUrl: './current-ld-plantwise.component.html',
  styleUrls: ['./current-ld-plantwise.component.css'],
  providers: [DatePipe]
})
export class CurrentLdPlantwiseComponent implements OnInit {
  ...
  ngOnInit() {
    this.titleService.setTitle("Plant Wise LD")
  ...
  loadOtdPlantData(){
    this.http.get(this.apiUrl + 'dashboard/dashboard?type=load-ld-Plant').subscribe(response => {
      var data = response;
