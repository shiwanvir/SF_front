...
@Component({
  selector: 'app-load-rm-plantwise-otd-lastyear',
  templateUrl: './load-rm-plantwise-otd-lastyear.component.html',
  styleUrls: ['./load-rm-plantwise-otd-lastyear.component.css'],
    providers: [DatePipe]
})
export class LoadRmPlantwiseOtdLastyearComponent implements OnInit {
  ...
  ngOnInit() {
    this.titleService.setTitle("RM Plant Wise OTD")
  ...
  loadOtdPlantData(){
    this.http.get(this.apiUrl + 'dashboard/dashboard?type=load-plantwise-lastyear-rm-otd').subscribe(response => {
      var data = response;
