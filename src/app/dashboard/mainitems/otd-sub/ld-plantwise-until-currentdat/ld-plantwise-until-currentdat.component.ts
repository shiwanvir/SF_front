...
@Component({
  selector: 'app-ld-plantwise-until-currentdat',
  templateUrl: './ld-plantwise-until-currentdat.component.html',
  styleUrls: ['./ld-plantwise-until-currentdat.component.css'],
  providers: [DatePipe]
})
export class LdPlantwiseUntilCurrentdatComponent implements OnInit {
  ...
  ngOnInit() {
    this.titleService.setTitle("Plant Wise TD")
  ...
  loadOtdPlantData(){
    this.http.get(this.apiUrl + 'dashboard/dashboard?type=load-ld-Plantwise-until-currentdate').subscribe(response => {
      var data = response;
